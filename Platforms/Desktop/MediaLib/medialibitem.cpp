...
mediaLibItem::mediaLibItem(QWidget *parent, bool bAllSongs):
    QWidget(parent), ui(new Ui::mediaLibItem) {
    ui->setupUi(this);
    ...
    playlistLabel->setText("Media library");
...
void mediaLibItem::setFocus() const {
...
void mediaLibItem::mousePressEvent(QMouseEvent *event) {
    ...
    if (event->button() == Qt::LeftButton and (!playlistPath.isEmpty() or bMediaLib)) {
